...
def generate_required_city_graph(city_name, network, remove_highway_label=False):
    ...
    X_V = nd.array(X_V_np, ctx=gpu(0))
    X_E = nd.array(X_E_np, ctx=gpu(0))
    X_B = nd.array(X_B_np, ctx=gpu(0))
    y = nd.array(y_np, ctx=gpu(0))
...
def mask_neighborhoods(neighborhoods_list, is_dual=False):
    ...
    neighborhoods_array = nd.zeros(shape=shape, dtype=np.int32, ctx=gpu(0))
    mask = nd.zeros(shape=shape, ctx=gpu(0))
