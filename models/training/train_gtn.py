...
traj_train = pd.read_pickle(
    f"../../datasets/trajectories/{city}/traj_train_test_split/train_{seed}.pkl"
)
...
traj2vec = Traj2VecModel(data_rest, network, device=device, emb_dim=128)
traj2vec.load_model(f"../model_states/traj2vec/model_base_{city}.pt")
gtc = GTCModel(data_rest, device, network, None, adj=adj)
gtc.load_model(f"../model_states/gtc/model_base_{city}.pt")  # no road
...
torch.save(
    ...
    os.path.join("../model_states/gtn/" + f"/model_base_{city}_{seed}.pt"),
)
