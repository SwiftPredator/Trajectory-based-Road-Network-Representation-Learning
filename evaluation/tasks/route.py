...
class RoutePlanning(Task):
    ...
    def __init__(
        ...
        self.train_loader = DataLoader(
            RP_Dataset(train, network, comp_ratio),
            collate_fn=RP_Dataset.collate_fn_padd,
            batch_size=batch_size,
            shuffle=True,
        )
        self.eval_loader = DataLoader(
            RP_Dataset(test, network, comp_ratio),
            collate_fn=RP_Dataset.collate_fn_padd,
            batch_size=batch_size,
        )
...
class RP_Dataset(Dataset):
    def __init__(self, data, network, comp_ratio: float):
        self.X = (
            data["seg_seq"].swifter.apply(lambda x: [x[0], x[-1]]).values
        )  # get start and end
        self.y = (
            data["seg_seq"].swifter.apply(lambda x: x[1:-1]).values
        )  # get in between as label
    ...
    def collate_fn_padd(batch):
        ...
        label = [[map[0][l] for l in ls] for ls in label]
        # print(label)
        l_max = max(max(x) for x in label) + 1
        # convert to one hot encoding
        one_hot = torch.zeros(len(label), len(map[0].keys()), dtype=torch.float)
        # print(one_hot.shape)
        for i in range(len(label)):
            one_hot[i, label[i]] = 1
        # one_hot[torch.arange(len(label)), label] = 1
        # print(one_hot.nonzero())
        return (
            data,
            one_hot,
            lengths,
...
class RP_LSTM(nn.Module):
    ...
    def predict(self, loader, emb):
        ...
        for X, y, lengths, mask, map in loader:
            ...
            emb_batch = emb_batch.to(self.device)
            y = y.to(self.device)
            yh = self.soft(self.forward(emb_batch, lengths))
            for i in range(yh.shape[0]):
                labels = y[i].nonzero().sort()[0]
                out = yh[i].topk(k=len(labels), sorted=True)[1]
                yhs.extend(out.tolist())
